# -*- coding: utf-8 -*-
"""
Rellena la columna Región según la Comuna en candidatos_final.xlsx.
Solo actualiza filas donde Región esté vacía o incorrecta.
Salida: sobreescribe candidatos_final.xlsx
"""

import openpyxl, unicodedata
from pathlib import Path

RUTA = Path(r'C:\Users\ignacio.cruz\OneDrive - Universidad de Chile\Escritorio\TRABAJO\06. Colina\11. Datos\12. Proyecto Jorge\candidatos_final.xlsx')

# ── Mapa comuna → región (todas las regiones de Chile) ────────────────────────
...
REGION_MAP = {
    ...
    # Localidades menores / variantes
    "Confluencia":       "Región de La Araucanía",
    "Mininco":           "Región del Biobío",
    "Ñipas":             "Región de Los Lagos",
...
REGION_NORM = {_norm(k): v for k, v in REGION_MAP.items()}

def buscar_region(comuna):
    if not comuna:
        return ''
    key = _norm(comuna)
    # Búsqueda exacta normalizada
    if key in REGION_NORM:
        return REGION_NORM[key]
    # Búsqueda parcial (por si la comuna tiene texto extra)
...
i_com = headers.index('Comuna')
i_reg = headers.index('Región')

actualizadas = 0
sin_match = set()

for row in ws.iter_rows(min_row=2):
    com = str(row[i_com].value or '').strip()
    reg_actual = str(row[i_reg].value or '').strip()

    if not com:
        continue

    nueva_reg = buscar_region(com)
    if nueva_reg and nueva_reg != reg_actual:
        row[i_reg].value = nueva_reg
        actualizadas += 1
    elif not nueva_reg:
        sin_match.add(com)
...
print(f"Regiones actualizadas: {actualizadas}")
if sin_match:
    print(f"\nComunas sin match ({len(sin_match)}) - revisar manualmente:")
    for c in sorted(sin_match):
        print(f"  {repr(c)}")
else:
    print("Todas las comunas encontradas.")
